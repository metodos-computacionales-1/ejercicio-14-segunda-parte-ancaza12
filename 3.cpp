//Código tomado de https://raw.githubusercontent.com/JoseMontanaC/Laboratorio-Metodos-Computacionales/master/c_tests/Rungekutta_Nvars.cpp
...
int main(void)
{
  ofstream fout("fric.dat");
  double x = 1, v = 0;
  double time;
  double y[N] = {1, 0};
  for(time = 0.37; time <= 20; time += DeltaT) {
    cout << time << "\t" << x << "\t" << v << endl;
    rk4(time, DeltaT, x, v);
...
double f(double t, double y[], int i)
{
  if (0 == i) {
    ...
    double gamma = 0.3;
    return (-K/M)*pow(y[0], LAMBDA ) - (gamma*y[1]) ;
  }
  else {
